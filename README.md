...
- 积分与排行榜：每局按游戏规则结算积分，写入个人档案；总排行榜实时更新、数据落盘持久保存
...
1. 房主点「＋ 创建房间」，把 6 位房间码发给对方；
2. 对方在同一页面点「加入房间」输入房间码；
3. 房主选择井字棋或五子棋开始，双方各自在自己设备上落子。
...
| 五子棋 | 胜 +15 / 负 +3 / 平 +8 |
| 飞行棋 | 冠军 +30 / 其他 +5 |
| 迷你大富翁 | 按结算排名 +40 / +20 / +10 / +5 / +2 |
| 弹珠跳棋 | 冠军 +25 / 其他 +4 |

对局结束自动结算并弹出提示；联机对局中双方各自结算自己的档案。

### 排行榜

- 大厅底部"总排行榜"显示总积分排名（头像 / 昵称 / 各游戏得分）
...
| 服务端→房主 | `{"type":"peer_joined"}` | 对方已加入 |
| 房主→服务端 | `{"type":"start","payload":{"game":"gomoku"}}` | 开始游戏 |
| 服务端→双方 | `{"type":"started","game":"gomoku"}` | 进入对局 |
