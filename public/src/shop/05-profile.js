...
function renderProfilePopup(p, isMe){
  ...
  const coinLine = el('div','pmeta profile-currency-summary');
  const balance=testAdmin&&typeof testAdminCurrencyText==='function'?testAdminCurrencyText(account):currencyAmountText(p.coins || 0);
  coinLine.appendChild(currencyAmountNode(p.coins || 0,{formattedText:String(balance)}));
  coinLine.appendChild(el('span', null, t('profile_summary_meta', p.total || 0, t(p.online ? 'online_label' : 'offline_label'))));
  ...
  GAME_KEYS.filter(k => ((p.played && p.played[k]) || 0) > 0).forEach(k => {
    ...
    s.textContent = GAMES[k].icon + ' ' + formatGamesCount((p.played && p.played[k]) || 0) + (title ? ' · ' + title.badge + ' ' + t(title.nameKey) : '');
    stats.appendChild(s);
  });
