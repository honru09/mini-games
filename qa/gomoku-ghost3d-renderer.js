...
class FakeGeometry {
  ...
  setFromPoints(points) {
    this.points = points.map(point => point.clone ? point.clone() : point);
    return this;
  }
...
class FakeLine extends FakeLineSegments {}
      ...
      const timeline = {
        ...
        complete() {
          this.children.forEach(child => {
            ['x', 'y', 'z', 'scale', 'opacity'].forEach(key => {
...

{
  ...
  check(highTimeline.children.length === 5, 'HIGH motion stays inside the camera/aim/stone/reused-impact-ring timeline budget');
  check(highTimeline.labels.map(item => item[0]).join(',') === 'focus,place,impact,settled', 'HIGH motion uses the shared semantic camera and impact labels');
  ...
  check(balancedTimeline.children.length === 5, 'BALANCED shortens the same bounded semantic camera/impact choreography');
...
{
  const harness = makeHarness(true);
  const mount = new FakeElement(420, 360);
  const adapter = harness.api.createGomoku3DAdapter(validOptions(mount, {
    onInput() {}, onContextLost() {}, onError() {}, onReady() {}
  }));
  adapter.mount({ quality:'HIGH', reducedMotion:false });
  const winningLine = [3,4,5,6,7].map(col => ({ row:7, col }));
  const stones = winningLine.map((cell,index) => ({ ...cell, player:index % 2 }));
  adapter.render({ revision:1, board:{ size:15, stones, lastMove:stones.at(-1), winningLine }, process:{ stage:'line' }, view:{ quarterTurns:0 } });
  const renderer = harness.state.renderers[0];
  const boardGroup = renderer.lastScene.children.find(child => child instanceof FakeGroup);
  const resultLine = boardGroup && boardGroup.children.find(child => child instanceof FakeGroup &&
    child.children.some(candidate => candidate instanceof FakeMesh && candidate.material && candidate.material.options.color === 0xf1b640));
  const resultRibbon = resultLine && resultLine.children[0];
  check(resultLine && resultLine.visible === true && resultRibbon instanceof FakeMesh && resultRibbon.scale.x > 4,
    'winning-line frame creates one reusable world-space result ribbon from the semantic endpoints');
  check(adapter.motion({ type:'winning_line', winningLine, revision:1 }) === true,
    'semantic winning-line event starts the result choreography');
  const resultTimeline = harness.state.timelines.at(-1);
  check(resultTimeline.labels.map(item => item[0]).join(',') === 'read,reveal,settled' && resultTimeline.children.length === 5,
    'result choreography uses one bounded camera/aim/line timeline');
  check(typeof renderer.loop === 'function','result choreography renders only while its finite timeline is active');
  resultTimeline.complete();
  check(renderer.loop === null && resultRibbon.material.opacity === .92,
    'completed result choreography settles into a visible static winning line');
  adapter.render({ revision:2, board:{ size:15, stones, lastMove:stones.at(-1), winningLine }, process:{ stage:'terminal' }, terminal:true, view:{ quarterTurns:0 } });
  adapter.environment({ reducedMotion:true });
  const beforeStaticResult = harness.state.timelines.length;
  check(adapter.motion({ type:'terminal', outcome:'win', winningLine, revision:2, reducedMotion:true, instant:true }) === true &&
    harness.state.timelines.length === beforeStaticResult && resultLine.visible === true && resultRibbon.material.opacity === .92,
  'reduced-motion terminal uses the same readable result pose without a timeline');
  adapter.dispose();
}
