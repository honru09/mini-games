...
check('权威请求显示可取消的 accessible loading dialog',
  /function beginPublicProfileRequest\(uid,requestId\)/.test(profile) &&
  /t\('profile_loading'\)/.test(profile) &&
  /setupAccessibleOverlayDialog\(bd,card,cancel/.test(profile) &&
  /closeProfileLoading/.test(profile),
  '缺少加载态、取消或清理');
check('Profile 请求以 requestId + UID 的有序本地记录绑定并能处理 null/not-found',
  /pendingPublicProfile/.test(online) && /publicProfileRequests/.test(online) &&
  /finishPublicProfileRequest\(matchedProfile,request\)/.test(online) &&
...
check('加载态不会把取消后的迟到响应重新打开',
  ...
  /request&&request\.active/.test(online) && /const matchedProfile=profile&&String\(profile\.uid\|\|''\)===request\.targetUid\?profile:null/.test(online),
