...
const smallLudoFrameRule = '@media(max-width:420px){';
const ludoPortraitRule = '@media(max-width:720px) and (orientation:portrait){';
const smallLudoFrameIndex = css.indexOf(smallLudoFrameRule);
const ludoPortraitIndex = css.indexOf(ludoPortraitRule);
const ludoPortraitBlock = ludoPortraitIndex >= 0 ? css.slice(ludoPortraitIndex, css.indexOf('@media(max-height:600px) and (orientation:landscape){', ludoPortraitIndex)) : '';
const ludoPortraitArenaCss = /\.ludo-wave-b-arena\{grid-template-columns:minmax\(0,1fr\);justify-content:stretch\}/.test(ludoPortraitBlock);
check('390px Ludo portrait arena gives the stage a full-width explicit single grid track',
  ludoPortraitArenaCss && /\.ludo-wave-b-stage[^}]*width:100%/.test(css));
check('390px Ludo portrait board frame is content-width square and cannot force board-area horizontal scroll',
  smallLudoFrameIndex >= 0 && ludoPortraitIndex > smallLudoFrameIndex && ludoPortraitArenaCss &&
  /\.ludo-wave-b-board-frame\{width:100%;height:auto;min-height:0;aspect-ratio:1\}/.test(ludoPortraitBlock));
