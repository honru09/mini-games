...
const surfaceEntry = read('public/surface-motion-entry.js');
const stageEntry = read('public/game-stage-motion-entry.js');
...
check(/from '\.\/vendor\/gsap\/3\.15\.0\/esm\/index\.js'/.test(surfaceEntry), 'surface entry imports the local versioned index');
check(/from '\.\/vendor\/gsap\/3\.15\.0\/esm\/index\.js'/.test(stageEntry), 'Game Stage DOM entry imports the local versioned index');
...
check(/CSSPlugin/.test(surfaceEntry) && /CSSPlugin/.test(stageEntry), 'all DOM tween adapters explicitly require the official CSSPlugin export');
