...
const ENTRY_FILE = 'xiangqi-entry.js';
const ENTRY_EXPORTS = ['isXiangqi3DSupported', 'createXiangqi3DAdapter'];
const serviceWorker = fs.readFileSync(path.join(ROOT, 'public', 'sw.js'), 'utf8');
const loader = fs.readFileSync(path.join(ROOT, 'public', 'src', 'core', '14-game-module-loader.js'), 'utf8');
const caller = fs.readFileSync(path.join(ROOT, 'public', 'src', 'games', 'xiangqi.js'), 'utf8');
...
assert.match(entry.toString('utf8'), /from '\.\.\/vendor\/three\/r185\/build\/three\.module\.js'/,
  'Xiangqi renderer keeps its Three import same-origin and relative');
assert.match(entry.toString('utf8'), /from '\.\.\/vendor\/gsap\/3\.15\.0\/esm\/gsap-core\.js'/,
  'Xiangqi renderer keeps its GSAP core import same-origin and relative');
...
assert.match(serviceWorker, /const RENDERER_CACHE_VERSION='ghost-game-renderer-v17-20260819';/,
  'Xiangqi renderer entries must use the dedicated verified renderer cache');
assert.ok(serviceWorker.includes(`gameId:'${GAME_ID}'`) && serviceWorker.includes(`url:'${rendererUrl}'`) &&
  ...
  'the Xiangqi SW descriptor must bind its exact version URL, full digest, and Adapter Interface');
assert.ok(loader.includes(`sha256: '${sha256}'`) && loader.includes(`primary: '${rendererUrl}'`),
  'the Xiangqi Loader descriptor must agree with the service-worker identity');
assert.match(caller, /GameModuleLoader\.load\('xiangqi',\s*\{\s*resource\s*:\s*'renderer'\s*\}\)/,
  'Xiangqi must request its optional renderer through the closed GameModuleLoader seam');
assert.ok(!/import\(\s*['"]\.\/three\/xiangqi-entry\.js/.test(caller),
  'Xiangqi callers must not retain an arbitrary direct renderer import path');
...
for (const forbidden of ['three/xiangqi-entry.js', 'vendor/three/', 'vendor/gsap/']) {
  assert.ok(!shell.some(item => String(item).includes(forbidden)),
    `Xiangqi renderer runtime stays lazy and out of the install shell: ${forbidden}`);
}
...
assert.match(serviceWorker, /async function warmRenderer\(descriptor\)/,
  'the bounded Xiangqi warmup path remains available without putting the renderer in the install shell');

console.log('XIANGQI_GHOST3D_CACHE_ALL_PASS');
