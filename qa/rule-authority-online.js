...
const CAPS=['tetris-rule-v2','xiangqi-rule-v2','monopoly-rule-v2'];
...
async function main(){
  ...
  try{
    ...
    check('Tetris v2 Online：Action→Server State/Hash 广播',ts1.payload.players[0].hash&&ts1.payload.players[0].hash===ts2.payload.players[0].hash&&ts1.payload.players[0].seq===1&&ts1.payload.players[0].state.pieces>=1,JSON.stringify({host:ts1.payload.players[0],guest:ts2.payload.players[0]}));
    host.send('tetris_action',{matchId:tetris.matchId,seq:1,action:{type:'hard_drop'}});const tErr=await host.wait('gameplay_error');check('Tetris v2 Online：重复 seq 返回统一错误码',tErr.payload.code==='ERR_DUPLICATE_ACTION');
    ...
    check('Tetris v2 Online：重连快照含完整规则状态',rejoined.payload.matchId===tetris.matchId&&rejoined.payload.tetrisRuleSnapshot&&rejoined.payload.tetrisRuleSnapshot.protocol==='tetris-rule-v2'&&rejoined.payload.tetrisRuleSnapshot.players[0].seq===1&&rejoined.payload.tetrisRuleSnapshot.players[0].state.pieces>=1);
