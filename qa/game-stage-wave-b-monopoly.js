...
try {
  ...
  // A small real board cannot fit three labels in each of the 24 cells. The
  // render seam must choose a visual density from its DOM box, keep semantic
  // tile text as aria/title data, and reuse (not duplicate) the live dice.
  const compact = createHarness(makeStorage(null));
  const compactFrame = compact.area.querySelector('.monopoly-wave-b-board-frame');
  compactFrame.clientWidth = 296;
  compact.area.clientHeight = 440;
  compact.game.onRestore(restoredState(2, 'buy'));
  const compactBoard = compact.area.querySelector('.monopoly-wave-b-board');
  const compactCells = compactBoard.querySelectorAll('[data-monopoly-cell]');
  const compactCurrent = compactBoard.querySelector('[data-monopoly-cell-current="true"]');
  const compactDice = compact.extra.querySelector('[data-monopoly-control="dice"]');
  check('compact board keeps 24 spaced DOM cells, one current cell, and complete accessible property text',
    compactBoard.dataset.monopolyDensity === 'compact' && compactCells.length === 24 &&
    compactCells.every(node => node.dataset.monopolyCellDensity === 'compact' && !!node.querySelector('.monopoly-cell-label')) &&
    compactCurrent && compactCurrent.getAttribute('aria-current') === 'location' &&
    compactCells[2].getAttribute('role') === 'img' &&
    /¥300/.test(compactCells[2].getAttribute('aria-label') || '') && compactCells[2].title === compactCells[2].getAttribute('aria-label'),
    JSON.stringify({density:compactBoard.dataset.monopolyDensity,cells:compactCells.length,cellDensity:compactCells[2]&&compactCells[2].dataset.monopolyCellDensity,current:compactCurrent&&compactCurrent.getAttribute('aria-current'),role:compactCells[2]&&compactCells[2].getAttribute('role'),label:compactCells[2]&&compactCells[2].getAttribute('aria-label'),title:compactCells[2]&&compactCells[2].title,hasLabel:compactCells[2]&&!!compactCells[2].querySelector('.monopoly-cell-label')}));
  check('compact board moves the existing roll control to Command while the dice row remains in the board',
    compactDice && compactDice.parentNode === compact.extra.querySelector('.monopoly-wave-b-command') &&
    compactDice.dataset.monopolyCompactControl === 'command' &&
    compactBoard.querySelector('.monopoly-wave-b-dice-row') && !compactBoard.querySelector('[data-monopoly-control="dice"]'));
  compactFrame.clientWidth = 520;
  compact.area.clientHeight = 560;
  compact.game.onRestore(restoredState(2, 'buy'));
  check('returning to a wide board restores full density and the same roll button to the centre',
    compactBoard.dataset.monopolyDensity === 'full' && compact.area.querySelector('[data-monopoly-control="dice"]') === compactDice &&
    compactDice.parentNode && compactDice.parentNode.classList.contains('monopoly-wave-b-center'),
    JSON.stringify({density:compactBoard.dataset.monopolyDensity,areaDice:compact.area.querySelector('[data-monopoly-control="dice"]')===compactDice,parent:compactDice.parentNode&&compactDice.parentNode.className,commandDice:compact.extra.querySelector('[data-monopoly-control="dice"]')===compactDice}));
  compact.game.destroy();

  const micro = createHarness(makeStorage(null));
  const microFrame = micro.area.querySelector('.monopoly-wave-b-board-frame');
  microFrame.clientWidth = 170;
  micro.area.clientHeight = 300;
  micro.game.onRestore(restoredState(2, 'buy'));
  const microBoard = micro.area.querySelector('.monopoly-wave-b-board');
  const microCells = microBoard.querySelectorAll('[data-monopoly-cell]');
  const microDice = micro.extra.querySelector('[data-monopoly-control="dice"]');
  check('micro landscape budget keeps the board finite, makes every cell compact, and retains the one Command roll control',
    microBoard.dataset.monopolyDensity === 'micro' && microCells.length === 24 &&
    microCells.every(node => node.dataset.monopolyCellDensity === 'micro' && Number.parseInt(node.style.width, 10) >= 14 && Number.parseInt(node.style.width, 10) <= 15) &&
    microDice && microDice.parentNode === micro.extra.querySelector('.monopoly-wave-b-command') && microDice.dataset.monopolyCompactControl === 'command');
  micro.game.destroy();
