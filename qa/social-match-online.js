...
// The authority runs in a child process. Date.now() is a wall clock rather
// than a cross-process monotonic clock, so Windows can expose a tiny skew at
// the millisecond boundary even though the event is signed after send().
const AUTHORITY_CLOCK_SKEW_MS = 1000;
const CAPABILITIES = [
...
async function main() {
  ...
  const observedAuthorityAt = Date.now();
  const authorityValid = authority.protocol === CAPABILITY && authority.senderUid === a.uid && authority.player === 0 && authority.targetSeat === 1 && Number.isInteger(authority.createdAt) && authority.createdAt >= beforeAuthority - AUTHORITY_CLOCK_SKEW_MS && authority.createdAt <= observedAuthorityAt + AUTHORITY_CLOCK_SKEW_MS && authority.senderUid !== 'forged-sender' && authority.player !== 99 && authority.createdAt !== 0;
  check('sender/player/time/protocol are server authoritative', authorityValid, authorityValid ? '' : JSON.stringify({ protocol:authority.protocol, senderMatches:authority.senderUid === a.uid, player:authority.player, targetSeat:authority.targetSeat, createdAt:authority.createdAt, beforeAuthority, observedAt:observedAuthorityAt }));
