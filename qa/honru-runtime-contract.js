...
// Honru Runtime Integration P2 专项契约：默认关闭、失败回退、生命周期与规则隔离。
...
function validManifest(){
  ...
  return { assets:[{
    asset_id:'P-HONRU-STATES-V1', runtime_id:'honru', status:'ready',
    feature_flags:{ operator:'all', enabled_value:'1', default_enabled:false, ids:['mg_art_honru_states_v1','mg_art_honru_game_reactions_v1'] },
...
async function run(){
  ...
  for (const master of flagValues){
    for (const game of flagValues){
      ...
      const expected = master === '1' && game === '1';
      if (enabled() !== expected) matrixOk = false;
    }
  }
  check('双旗标完整矩阵：仅两个字符串 1 同时存在时启用', matrixOk);
  ...
  runtime.storage.removeItem('mg_art_honru_game_reactions_v1');
  runtime.resolveDecode();
  const revoked = await pending;
  check('资源加载期间撤销任一旗标会取消激活并恢复 v1', revoked === false && pendingImage.src === 'assets/brand/honru-mascot-v1.svg' && pendingImage.dataset.honruState === 'fallback',
    JSON.stringify({ revoked, src:pendingImage.src, state:pendingImage.dataset.honruState }));
  runtime.storage.setItem('mg_art_honru_game_reactions_v1', '1'); runtime.setDecodeMode('resolve');
  ...
  check('同状态高频事件命中冷却并保留原节点', playfulNode && playfulNode.dataset.honruState === 'playful' && runtime.document.getElementById('honru-game-reaction') === playfulNode);
  ...
  check('观众结算不显示个人胜负 Honru 语义', spectatorResult === false && !runtime.document.getElementById('honru-game-reaction'));
  ...
  check('六款游戏规则 snapshot 在双闸门前后逐字一致', snapshotIsolation);
