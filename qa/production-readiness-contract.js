...
async function main(){
  ...
  const sameProjectRestoreDrillRejected =
    /if\s*\(\s*\$sourceIdentity\.ProjectRef\s*-eq\s*\$targetIdentity\.ProjectRef\s*\)\s*\{\s*throw\s+'[^']*同一(?:\s*Supabase)?\s*项目[^']*'\s*\}/.test(ops) &&
    !/\$sourceIdentity\.ProjectRef\s*-eq\s*\$targetIdentity\.ProjectRef\s*-and\s*\$sourceIdentity\.Database\s*-eq\s*\$targetIdentity\.Database/.test(ops);
  check('Production Ops：恢复演练同一 Supabase 项目无条件拒绝、隔离并有破坏确认',/Get-SupabaseDatabaseIdentity/.test(ops)&&sameProjectRestoreDrillRejected&&/I_UNDERSTAND_EPHEMERAL_TARGET_WILL_BE_REPLACED/.test(ops));
  ...
  check('PWA：HTML network-first、静态版本缓存且 API/WS/Authorization 不缓存',/networkFirstNavigation/.test(sw)&&/CACHE_VERSION/.test(sw)&&/ghost-game-192\.png/.test(sw)&&/ghost-game-512\.png/.test(sw)&&/authorization/.test(sw)&&/api/.test(sw)&&/ws/.test(sw)&&/request\.mode==='navigate'/.test(sw)&&/content-type/.test(sw)&&/text\\\/html/.test(sw));
  const shellLiteral=(/const SHELL=\[([^\]]*)\]/.exec(sw)||[])[1]||'';
  check('PWA：三语言离线词典使用校验安装与精确同源运行时白名单',
    ['zh-CN','en-US','uk-UA'].every(lang=>sw.includes(`./locales/${lang}.json`))&&
    /LOCALE_URLS=new Set/.test(sw)&&/offlineLocaleRequest\(request,url\)/.test(sw)&&
    /installShell\(\)/.test(sw)&&/for\(const file of LOCALE_FILES\)/.test(sw)&&
    /if\(!validLocaleResponse\(response\)\)throw new Error\('invalid_locale_response'\)/.test(sw)&&
    /await caches\.delete\(CACHE_VERSION\)/.test(sw)&&!shellLiteral.includes('/locales/')&&
    /networkFirstLocale\(request\)/.test(sw)&&/fetch\(request,\{cache:'no-cache'\}\)/.test(sw)&&
    /application\\\/json/.test(sw)&&
    /request\.method==='GET'&&url\.search===''&&LOCALE_URLS\.has\(url\.href\)/.test(sw)&&
    !/CACHEABLE_DESTINATIONS=new Set\([^\n]*['"]json['"]/.test(sw)&&
    /CACHEABLE_DESTINATIONS\.has\(request\.destination\)&&!url\.pathname\.toLowerCase\(\)\.endsWith\('\.json'\)/.test(sw));
  ...
  const artContract=read('requirements/active/production-readiness-sprint-p0-20260809/contract.md');
  ...
  check('Honru 可选咨询：Reviewer B 与 Golden Set 当前未签字状态保持真实',
    /PENDING|BLOCKED_EXTERNAL/.test(reviewerB)&&/姓名：________________/.test(reviewerB)&&
    /PENDING|DO_NOT_ENABLE/.test(goldenDecision)&&/用户 Golden Set 批准 \| 未签字/.test(goldenDecision)&&
    !/\b(?:IP|LEGAL|GOLDEN_SET)_?PASS\b/i.test(reviewerB+goldenDecision));
  check('Honru 所有者清除：可选咨询不再阻塞开发或 runtime',
    artContract.includes('OWNER_AUTHORIZED_ART_CLEARANCE')&&
    artContract.includes('OPTIONAL_ADVISORY_EVIDENCE')&&
