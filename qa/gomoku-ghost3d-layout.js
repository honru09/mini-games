...
const fallbackCss = /\.gomoku-wave-b-board-frame\[data-ghost3d-ready="true"\] \.gomoku-wave-b-board\{[^}]*opacity:0[^}]*pointer-events:none/.test(TEMPLATE);
const metaCss = /\.gomoku-wave-b-meta\{[^}]*position:absolute[^}]*z-index:2/.test(TEMPLATE);

check('3D overlay atomically replaces DOM paint after readiness and keeps a strict pointer fallback', slotCss && readyCss && fallbackCss);
