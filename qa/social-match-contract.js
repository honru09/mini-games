...
check('public Seat exposes only the frozen identity/presentation contract',
  publicSeatKeys.every(key => new RegExp('\\b' + key + '\\b').test(serverPublicSeat)) &&
  privateSeatKeys.every(key => !new RegExp('\\b' + key + '\\b').test(serverPublicSeat)) &&
  /lang:\['zh-CN','en-US','uk-UA'\]\.includes\(seat\.lang\)/.test(serverPublicSeat) &&
  /playerCharacter:publicPlayerCharacter\(seat\.playerCharacter\)/.test(serverPublicSeat));
...
check('reduced-motion removes flight/bubble animation and flight code honors the guard',
  ...
  /item\.kind!==\'emoji\'\|\|prefersReducedMotion\(\)/.test(shell) && /pointer-events:none/.test(template));
