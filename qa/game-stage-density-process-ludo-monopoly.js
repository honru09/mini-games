...
const monopolyRoll = sourceSegment(monopoly, 'function applyRoll(', 'function movePlayer(');
const monopolyPay = sourceSegment(monopoly, 'function pay(', 'function showChance(');
check('Monopoly roll emits exactly one accepted audio cue after its action guards',
  (monopolyRoll.match(/audioCue\('monopoly_roll'/g) || []).length === 1 &&
  monopolyRoll.indexOf("audioCue('monopoly_roll'") > monopolyRoll.indexOf("phase !== 'roll'"));
