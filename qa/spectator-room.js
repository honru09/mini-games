...
class Client{
  ...
  async wait(type,timeout=3000){const end=Date.now()+timeout;while(Date.now()<end){const index=this.messages.findIndex(item=>item.type===type);if(index>=0)return this.messages.splice(index,1)[0];await new Promise(resolve=>{this.waiters.push(resolve);setTimeout(resolve,30);});}throw new Error(this.name+' wait '+type+'; queued='+JSON.stringify(this.messages.slice(-8)));}
...
async function main(){
  ...
  try{
    ...
    guest.send('join',{room});await guest.wait('joined');host.send('select_game',{game:'gomoku'});const gomokuStarted=await host.wait('started');host.send('move',{r:7,c:7});await sleep(80);
    ...
    host.send('select_game',{game:'xiangqi'});const xiangqiStarted=await host.wait('started');host.send('move',{from:[6,0],to:[5,0],seq:1});const clock=await guest.wait('clock_state');
    ...
    host.send('end_game',{});await viewer2.wait('match_result');host.send('select_game',{game:'monopoly'});const monopolyStarted=await host.wait('started');
    ...
    host.send('leave',{});const closed=await viewer2.wait('peer_left');assert('Spectator：房间关闭收到通知',closed.payload.roomClosed===true);
