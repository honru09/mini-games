...
check('房间浏览摘要端到端保留房主 Avatar/Frame/Effect/NameFx',
  /hostFrame\s*:\s*hu\s*\?\s*\(hu\.frame/.test(lobbyServer) &&
  /hostEffect\s*:\s*hu\s*\?\s*\(hu\.effect/.test(lobbyServer) &&
  /hostNameFx\s*:\s*hu\s*\?\s*\(hu\.nameFx/.test(lobbyServer) &&
  /frame\s*:\s*r\.hostFrame/.test(lobbyClient) &&
  /effect\s*:\s*r\.hostEffect/.test(lobbyClient) &&
  /nameFx\s*:\s*r\.hostNameFx/.test(lobbyClient),
  'Lobby 仍将房主 frame/effect/nameFx 强制为 0');
