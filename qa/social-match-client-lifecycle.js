...
check('spectate_left clears spectator identity before target room wire while preserving same-socket capability',()=>{online.connected=true;online._authenticated=true;online.capabilities=new Set(['match-expression-v1','tournament-orchestrator-v1']);online.spectatorRoom='OLDSP1';online.room=null;online.player=null;online.isHost=false;online.isSpectator=true;online.roomInfo={room:'OLDSP1',spectator:true};online.onMessage({type:'spectate_left',payload:{room:'OLDSP1',reason:'tournament_rehome'}});assert.strictEqual(online.spectatorRoom,null);assert.strictEqual(online.room,null);assert.strictEqual(online.player,null);assert.strictEqual(online.isSpectator,false);assert(online.capabilities.has('match-expression-v1'));assert(online.capabilities.has('tournament-orchestrator-v1'))});
