...
function makeRuntime() {
  ...
  const labels = {
    ...
    presence_online:'Online', presence_offline:'Offline', region_unset:'Region unset', gender_hidden:'Hidden', profile_summary:'Summary %s %s %s', profile_summary_meta:'Games %s %s',
  ...
  const sandbox = {
    ...
    applyPremiumBackground() {}, releasePremiumBackground() { releases.premium++; },
    currencyAmountText(value) { return String(value) + ' G Coins'; },
    currencyAmountNode(value,options) { const node=el('span','currency-amount',options&&options.formattedText?String(options.formattedText):String(value)); node.setAttribute('role','img'); return node; },
