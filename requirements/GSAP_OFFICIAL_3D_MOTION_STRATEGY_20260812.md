...
本记录是既有 [`THREEJS_OFFICIAL_INTEGRATION_STRATEGY_20260812.md`](THREEJS_OFFICIAL_INTEGRATION_STRATEGY_20260812.md) 的 GSAP 配套决策，不替代原子台账。它服务既有 `TECH-049`（可替换 Ghost3D）与 `TECH-054`（GSAP Motion Governance），属于当前 `NOW_CLOSURE / CLOSE` 的准备工作。`GATE-DEVICE-BROWSER-NETWORK` 不阻塞本地开发，只让跨浏览器/真机/真实网络发布证据保持 `RELEASE_EVIDENCE_PENDING`；原创 Ghost-native 美术按 `OWNER_AUTHORIZED_ART_CLEARANCE` 接入可逆 runtime 候选，人工清稿、Reviewer B、IP/法律与逐资产 Golden Set 仅为可选风险咨询。
...
当前 Gomoku GSAP vertical slice 的本地 `implemented` 证据必须同时包含：官方 source/provenance 与 vendor hash、最小 module graph、lazy-load 和 failure fallback、生命周期/cleanup/context-loss/recover、reduced-motion static path、质量预算、bundle/perf 记录，以及 Rule/Authority/Replay/Reward/Input 无变化的回归。真实第二浏览器、Android/iPhone/Tablet、真实网络和最新浏览器可见矩阵保持 `RELEASE_EVIDENCE_PENDING`，不能由本策略或静态测试越级；正式原创美术按 `OWNER_AUTHORIZED_ART_CLEARANCE` 独立推进，不等待可选人工/IP/Golden Set 咨询。
