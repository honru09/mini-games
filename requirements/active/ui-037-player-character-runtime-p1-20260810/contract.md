...
1. ART-036：可先自动推进所有机器可验证的 runtime 工作；具体原创 Ghost-native 资产在 M0 North Star、稳定 ID/版本/SHA/provenance、机器技术/视觉/相似风险审查、Manifest、fallback、feature flag 与回滚齐全并取得 `OWNER_AUTHORIZED_ART_CLEARANCE` 后，可进入可逆 default-on runtime。自然人清稿、Reviewer B、IP/法律意见和逐资产 Golden Set 为可选咨询，不得伪造 PASS。
2. ECO-029：可继续 active catalog、价格权威、Resolver、local/fake adapter、失败回退与内部预览；`apply_purchase_v1`、真实并发/RLS、备份/恢复/非破坏回滚是生产持久化与发布证据，不得由本地 adapter 冒充，也不得阻塞上述本地开发。未启用正式经济时只显示 P0 default/fallback。
...
4. 浏览器/设备：四档本地浏览器、第二桌面浏览器、Android/iPhone/Tablet、reduced-motion 与真实网络整形完成前，不写成生产验证；状态为 `NON_BLOCKING_FOR_DEVELOPMENT / RELEASE_EVIDENCE_PENDING`。
