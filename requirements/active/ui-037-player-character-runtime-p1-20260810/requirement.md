...
状态：`CODE_FALLBACK_STATE_MATRIX_IMPLEMENTED / ART_AND_DEVICE_GATED`
...
对应唯一台账：`UI-037`、`GAME-045`；已完成前置：`GAME-012`、`SOC-031`。受阻前置：`ART-036` 的人工审批与 `ECO-029` 的正式经济/数据库事务。
...
- `ART-036` 必须先走完人工清稿、Reviewer B、IP Similarity Review 与用户 Golden Set；`ECO-029` 必须另行完成正式目录、`apply_purchase_v1`、并发、RLS、备份/恢复/回滚后，才可开始角色资源/收藏的运行时接入。
- 运行时新增静态文案必须同步 `zh-CN`、`en-US`、`uk-UA`，由 `t()` / i18n 属性渲染；用户昵称仅以 `textContent` 与 `data-i18n-raw` 处理。

## Completion definition

当前可确认的是**代码原生状态矩阵与安全 fallback 已实现**。UI-037/GAME-045 仍不能写成“正式角色美术完成”或“正式角色商城完成”；ART-036 人工审批、ECO-029 正式经济、第二浏览器/真机/网络整形仍是独立门禁。
