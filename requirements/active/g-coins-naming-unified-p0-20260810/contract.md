...
`gcoins-source-chroma-v1.png` 是最高质量模型生成的 source-only 色键母图，尺寸 1254×1254，状态 `reference-only`。抠图、Alpha/尺寸/污染/小尺寸、机器视觉/相似风险、Manifest/fallback/feature flag/回滚与 runtime 转换可自动推进；具体原创候选取得 `OWNER_AUTHORIZED_ART_CLEARANCE` 后可进入可逆 runtime。人工清稿、Reviewer B、IP/法律意见和逐资产 Golden Set 是可选咨询，不得伪造 PASS 或阻塞开发。
