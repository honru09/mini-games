...
- G Coins 图标源稿登记到素材库，但在人工清稿、Reviewer B、IP Review 和 Golden Set 审批完成前保持 `source-only/reference-only`，不得进入 runtime manifest。

## 非范围

- 不调整 Reward Resolver 数值、每日上限、商城价格、购买扣款、胜场/XP、Supabase schema/RPC、AI、Replay 或部署。
- 不把未经审批的 PNG 接入 `public/assets` 或默认 feature flag。
