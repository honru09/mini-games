...
> 当前裁决（2026-08-16）：本文件保留 Wave A 立项时的 `historical-as-of` 范围和当时默认关闭事实，不再作为当前美术准入门禁。原创 Ghost-native 资产按 `OWNER_AUTHORIZED_ART_CLEARANCE` 推进；Honru 九状态已是 Manifest-backed default-on 本地 runtime。人工清稿、Reviewer B、IP/法律意见与逐资产 Golden Set 均为 `OPTIONAL_ADVISORY_EVIDENCE`，不得阻塞开发或 runtime；发布仍需当前用户明确命令。
