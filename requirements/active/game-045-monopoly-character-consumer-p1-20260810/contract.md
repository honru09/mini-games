...
- 可在当前主线另立 runtime integration 任务并自动推进机器可验证工作。具体原创 Ghost-native 资产须先完成可审计源/许可/hash、M0 North Star 对齐、机器技术/视觉/相似风险审查、运行时 Manifest 条目、poster/静态 fallback、字节/内存预算、双主题/三语言/a11y/reduced-motion/资源失败 QA、feature flag 与一键回滚，取得 `OWNER_AUTHORIZED_ART_CLEARANCE` 后即可进入可逆 default-on 候选。人工清稿、Reviewer B、IP/法律意见和逐资产 Golden Set 是可选咨询，不得伪造 PASS，也不得阻塞 runtime 工作。
