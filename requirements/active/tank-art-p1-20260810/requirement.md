...
为坦克大战建立 Ghost Game 自有的高质量实体桌游/克制漫画视觉方向：四套可扩展坦克皮肤、可读的地图材质、基地与玩家角色露出规则，以及统一的运行时资源交付和 `OWNER_AUTHORIZED_ART_CLEARANCE` 入口。此阶段只产出 source-only 概念和合同；人工/IP/Golden Set 保留为可选咨询，不把尚未取得所有者清除的资源默认接入线上。
...
- 没有逐资产 `OWNER_AUTHORIZED_ART_CLEARANCE` 前，具体候选保持 draft/source-only；取得所有者清除后可以进入可逆 runtime 与可见验证。独立 Reviewer B、IP/法律意见和用户 Golden Set 是可选咨询，缺失不得阻塞开发，也不得伪造成已通过；`production-ready` 仍需真实设备/网络证据与当前用户明确发布命令。
