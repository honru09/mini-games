...
| 资产 | P0 历史纵切先用程序化 geometry/material；原创 Ghost-native GLB/GLTF 可在逐族取得 `OWNER_AUTHORIZED_ART_CLEARANCE` 后接入，GLTFLoader 与压缩 decoder 仍须完整模块图、性能、fallback 与回滚审计。人工/IP/Golden Set 是可选咨询。 |
...
P0 历史纵切**不接 GLB**：用 `BoxGeometry`/`CylinderGeometry`/基础材质/灯光验证共享 Host、camera、resize、quality、reduced-motion、语义输入、context-loss 与 dispose。后续不得把仍为 source/reference-only 的候选带入 runtime；但原创 Ghost-native 资产逐族取得 `OWNER_AUTHORIZED_ART_CLEARANCE` 后，即可在 Asset Manifest 中把稳定 Asset ID 映射到 GLB，并保留同一 fallback 与一键回滚，不等待可选人工/IP/Golden Set 咨询。
...
## 5. GLTFLoader/GLB 的传递依赖与所有者清除顺序
...
因此 **P0 程序化优先**是风险控制而非放弃 GLB：先验证 renderer seam 与 fallback；原创 Ghost-native 候选在 M0 North Star、稳定 ID/版本/SHA/provenance、机器技术/视觉/相似风险审查、Manifest、feature flag、fallback 与回滚齐全后取得 `OWNER_AUTHORIZED_ART_CLEARANCE`，即可接入 GLB 及其全部传递资产。人工清稿、Reviewer B、IP/法律意见和逐资产 Golden Set 是可选咨询；真机/第二浏览器/真实网络只保留发布证据待决。禁止用“GLTFLoader 能加载”冒充所有者清除或伪造人工/IP PASS。
...
| GLTF/decoder 传递网络、CSP、WASM/Worker、GPU 成本 | P0 程序化；原创资产取得 `OWNER_AUTHORIZED_ART_CLEARANCE` 后才 manifest 化 GLB 与 decoder，并测完整 graph。 |
| 颜色/曝光在设备上不一致 | 显式 color config；HDR/tone mapping 在所有者清除后可进入可逆 HIGH 候选，真机校准作为发布证据待决。 |
...
| 未取得所有者清除的 source art 进入 runtime | 未取得 `OWNER_AUTHORIZED_ART_CLEARANCE` 的候选只允许 source/default-off 技术预览；外部受限素材始终禁用；已清除原创 GLB 保留 fallback 与 kill switch。 |

P0 只有在以下证据齐全时才可称 `implemented`（而非 `3D_VISUAL_VERIFIED`）：Gomoku 入场才产生 Three 请求；WebGL2 gate 可见；程序化场景可玩且 Wave B 可回退；context loss/recover 与 dispose 循环通过；颜色/reduced-motion/CSP/MIME/SW 行为有记录；静态扫描证明 Three 类型没有越界。真实第二浏览器、Android/iPhone/Tablet 与真实网络仍是发布证据待决；原创美术按所有者清除轨道独立推进，可选 Golden Set 咨询缺失不阻塞开发，发布仍只接受当前用户明确命令。
