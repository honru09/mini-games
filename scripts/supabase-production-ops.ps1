...
switch ($Action) {
  ...
  'restore-drill' {
    ...
    if ($sourceIdentity.ProjectRef -eq $targetIdentity.ProjectRef -and $sourceIdentity.Database -eq $targetIdentity.Database) {
      throw '恢复演练目标与生产数据库身份相同；仅修改 host/user/query 不能作为隔离目标。'
    }
